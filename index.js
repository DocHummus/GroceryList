...
const logItemNames = () => {
  ...
  items.forEach((itm) => console.log(itm));
};
...
const capitalizeNames = () => {
  return items.map((item) => {
    const itemSlice = item.name.slice(0, 1).toUpperCase();
    const itemRest = item.name.slice(1);
    return {
      ...item,
      name: itemSlice + itemRest,
    };
...
function calculateTotalCartPrice() {
  ...
  let total = 0;
  for (let i = 0; i < cart.length - 1; i++) {
    const itemprice = findItemById(parseInt(cart[i])).price;
